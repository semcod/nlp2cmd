...
class ActionPlanner:
    ...
    def __init__(
        ...
        from nlp2cmd.canvas_planner.config import resolve_canvas_model, resolve_ollama_url

        self.ollama_url = resolve_ollama_url(ollama_url)
        self.model = resolve_canvas_model(model)
    ...
    def _try_canvas_decomposition(self, query: str) -> Optional[ActionPlan]:
        """Dynamic canvas decomposition via CanvasPlanningOrchestrator (LLM-first)."""
        if not _CANVAS_PLANNER_AVAILABLE:
            log.warning("canvas_planner not available for drawing query")
            return None
