...
# Let's search exactly for the index of the second `        elif action == "wait_for_canvas":`
count = 0
...
if start_dup != -1:
    for i in range(start_dup, len(lines)):
        # We need to stop deleting when we reach `elif action == "login":` which is at 4032 (wait was at 4028 but actually wait is NOT duplicated)
        # Wait, let's look at grep output:
        # 4028:        elif action == "wait":
